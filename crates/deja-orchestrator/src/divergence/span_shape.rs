...
fn in_namespace(span_name: &str, namespaces: &[String]) -> bool {
    namespaces.iter().any(|ns| span_name.starts_with(ns.as_str()))
}
...
fn project<'a>(
    ...
    let by_id: BTreeMap<u64, &ExecutionGraphNode> =
...
mod tests {
    ...
    fn instrumented_side() -> Vec<ExecutionGraphNode> {
        vec![
            ...
            node(3, Some(1), "ucs::flow_orchestration", &[("connector", "stripe"), ("flow", "Authorize")]),
            node(4, Some(3), "execute_connector_processing_step", &[]),
            node(5, Some(4), "ucs::build_request", &[]),
            node(6, Some(5), "connector::request_body", &[("connector", "Stripe"), ("flow", "Authorize")]),
        ]
    }
    ...
    fn a_changed_chokepoint_field_value_is_field_diverged_with_the_key_named() {
        ...
        for n in &mut rep {
            if n.span_name == "ucs::flow_orchestration" {
                n.fields
                    .insert("connector".to_owned(), serde_json::Value::String("adyen".to_owned()));
            }
        }
        let shape = compare(&refs(&rec), &refs(&rep), &ns()).expect("scored spans present");
    ...
    fn volatile_field_keys_do_not_diverge() {
        ...
        let shape = compare(&refs(&rec), &refs(&rep), &ns()).expect("scored spans present");
        assert!(shape.clean(), "request_id must not count as a field divergence");
    }
    ...
    fn same_named_siblings_pair_kth_with_kth_by_creation_order() {
        ...
        let side = |second_connector: &str| {
            vec![
                ...
                node(2, Some(0), "connector::request_body", &[("attempt", second_connector)]),
            ]
        };
        let rec = side("b");
        ...
        assert_eq!(shape.field_diverged, 1, "only the SECOND occurrence diverges");
        let div = shape
            .outcomes
            .iter()
            .find(|o| o.status == SpanShapeStatus::FieldDiverged)
