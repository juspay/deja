...
pub(crate) fn detect_with_plan(art: &RunArtifacts, graph_plan: &GraphScoringPlan) -> Scorecard {
    ...
    if !art.scored_span_namespaces.is_empty() {
        match art.record_graph.as_ref() {
            None => warnings_extra.push(
                "scored-span shape check skipped: record graph unavailable".to_owned(),
            ),
            Some(record_graph) => {
...
mod tests {
    ...
    fn instrumented_graph(corr: &str) -> Vec<deja_core::ExecutionGraphNode> {
        vec![
            ...
            scored_span(2, corr, Some(1), 1, "ucs::flow_orchestration", &[
                ("connector", "stripe"),
                ("flow", "Authorize"),
            ]),
            graph_span(3, corr, Some(2), 2, "execute_connector_processing_step"),
            scored_span(4, corr, Some(3), 3, "connector::request_body", &[
                ("connector", "Stripe"),
                ("flow", "Authorize"),
            ]),
        ]
    ...
    fn declaring_no_namespaces_keeps_the_scorecard_byte_identical() {
        ...
        assert_eq!(a, b, "undeclared check must leave no trace in the scorecard");
    ...
    fn matched_scored_spans_pass_and_surface_the_contract_section() {
        ...
        assert!(card.verdict.pass, "clean shape must pass: {}", card.verdict.reason);
        let outcome = &card.per_correlation[0];
        let shape = outcome.span_shape.as_ref().expect("span_shape section present");
    ...
    fn record_graph_unavailable_skips_the_check_with_a_warning() {
        ...
        let mut artifacts = with_scored_namespaces(art(
            vec![],
            vec![],
            vec![http(corr, true, vec![])],
        ));
        ...
        let card = detect(&artifacts);
        assert!(card.verdict.pass, "skip must not fail: {}", card.verdict.reason);
        assert!(card
            .warnings
            .iter()
    ...
    fn a_changed_chokepoint_field_fails_with_the_key_named() {
        ...
        assert!(!card.verdict.pass);
        assert!(
            card.verdict.reason.contains("1 scored-span field divergence(s)"),
            "reason: {}",
            card.verdict.reason
        );
