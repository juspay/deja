...
impl<T: KubeTransport> KubeApi<T> {
    ...
    /// GET a Deployment. The replay candidate's L1 config env is COPIED from the
    /// recorded system's own rendered Deployment (an artifact-only render of that
    /// system's chart at the record version), so the env is generated by that
    /// system's chart rather than reproduced here — including every `secretKeyRef`
    /// indirection, under the names its own config declares.
    pub fn get_deployment(&self, ns: &str, name: &str) -> Result<Value, KubeError> {
        let req = KubeRequest {
            method: "GET",
    ...
    /// GET a Secret — existence/metadata only, for the pre-launch check that every
    /// secret the copied env references is present. Never reads values: the
    /// candidate resolves them itself via `secretKeyRef` at pod start.
    pub fn get_secret(&self, ns: &str, name: &str) -> Result<Value, KubeError> {
        let req = KubeRequest {
            method: "GET",
