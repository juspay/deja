...
pub fn produce_tar(repo_dir: &Path, sha: &str) -> Result<Vec<u8>, String> {
    // The candidate's self-sufficient base router config rides alongside
    // migrations. The frozen image ships only superposition_seed/
    // payment_required_fields, so the Job boots the router `-f` this to get the
    // candidate's FULL (delta-complete) key set — config structure is a function
    // of the candidate, exactly like migrations; sbx-specific VALUES layer on as
    // ROUTER__* env at run time. `git archive` fails on a pathspec that matches
    // nothing, so include `config/docker_compose.toml` only when the ref has it.
    let config_blob = format!("{sha}:config/docker_compose.toml");
    let has_config = Command::new("git")
        .arg("-C")
        .arg(repo_dir)
        .args(["cat-file", "-e", &config_blob])
        .status()
        .map(|s| s.success())
        .unwrap_or(false);
    let mut pathspecs: Vec<&str> = vec!["migrations"];
    if has_config {
        pathspecs.push("config/docker_compose.toml");
    }
    let out = Command::new("git")
        .arg("-C")
        .arg(repo_dir)
...
/// `<top>/config/docker_compose.toml` → `config/docker_compose.toml`, the
/// candidate's self-sufficient base router config carried alongside `migrations/`
/// (config structure = f(candidate); see [`produce_tar`]). `<top>` must be a
/// single repo-root segment. `None` for anything else.
fn strip_to_root_config(path: &str) -> Option<String> {
    let idx = path.find("/config/docker_compose.toml")?;
    if path[..idx].contains('/') {
        return None;
    }
    Some(path[idx + 1..].to_owned())
    ...
    fn targz_producer_keeps_root_migrations_plus_config_and_rewrites_paths() {
        let targz = fake_codeload_targz(
            ...
            &[
                ...
                // The candidate's self-sufficient base config — kept alongside migrations.
        ...
        let extracted = fingerprint_from_migrations_dir(&dest.path().join("migrations"))
            .expect("fingerprint");
        assert_eq!(extracted.applied, fp.applied);
        // The candidate's base config rode along at the canonical root path (so the
        // Job can boot the router `-f config/docker_compose.toml`), and only the
        // root-level one — the crates/ decoy was dropped.
        assert!(
