...
pub fn render_lookup_table(
    ...
    render_lookup_table_excluding_correlations(
        recording_path,
        recording_id,
        policy_version,
        &HashSet::new(),
...
/// Walk a recording and produce a `LookupTable`, excluding every event whose
/// correlation id belongs to harness noise such as `/health`.
pub fn render_lookup_table_excluding_correlations(
    recording_path: &Path,
    recording_id: &str,
    policy_version: u32,
...
/// Extract one correlation's slice of a rendered table — the per-request table
/// the replay agent pushes to the router IMC before driving that request.
/// `None` selects ambient entries with no correlation id.
...
mod tests {
    ...
    #[test]
    fn renderer_excludes_configured_correlations() {
        let (_dir, path) = write_events(&[
            event("http_incoming", 0, serde_json::Value::Null),
            event("redis", 1, serde_json::Value::Null),
        ]);
        let excluded = std::collections::HashSet::from(["c-1".to_owned()]);

        let table =
            render_lookup_table_excluding_correlations(&path, "rec-1", 1, &excluded).unwrap();

        assert!(
            table.entries.is_empty(),
            "excluded health correlations must not leak lookup entries"
        );
    }
